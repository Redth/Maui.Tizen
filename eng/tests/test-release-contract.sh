...
case "$endpoint" in
  ...
  repos/*/rules/branches/*)
    cat <<'JSON'
[{"type":"pull_request","parameters":{"required_approving_review_count":1,"require_code_owner_review":true,"require_last_push_approval":true}},{"type":"deletion"},{"type":"non_fast_forward"},{"type":"required_linear_history"},{"type":"required_status_checks","parameters":{"strict_required_status_checks_policy":true,"required_status_checks":[{"context":"Build and test (no Tizen workload)"},{"context":"Hosted validation (no Tizen workload)"},{"context":"Tizen device lane (informational)"},{"context":"Verify imported history"},{"context":"Tizen workload availability (external gate)"}]}}]
JSON
    ;;
  repos/*/rulesets/1)
    cat <<'JSON'
