...
restore_current() {
  if [[ -n "$CURRENT_FILE" && -n "$CURRENT_BACKUP" && -f "$CURRENT_BACKUP" ]]; then
    cp -p "$CURRENT_BACKUP" "$CURRENT_FILE"
  fi
  CURRENT_FILE=""
  CURRENT_BACKUP=""
}
...
# These negative controls mutate source files that the already-built source tests inspect at
# runtime. Build once from the pristine tree, then run every mutation with --no-build so a
# deliberately incomplete source closure cannot fail compilation before its guard executes.
"$DOTNET" build tests/Maui.Tizen.SourceTests/Maui.Tizen.SourceTests.csproj \
  -c Release -v:minimal >"$SCRATCH/source-build.log" 2>&1
