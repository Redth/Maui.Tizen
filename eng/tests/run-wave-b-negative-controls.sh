...
restore_current() {
  if [[ -n "$CURRENT_FILE" && -n "$CURRENT_BACKUP" && -f "$CURRENT_BACKUP" ]]; then
    local restore_path="$CURRENT_FILE.wave-b-restore-$LOCK_TOKEN"
...
recover_stale_lock() {
  ...
  if [[ "$owner_worktree" != "$WORKTREE_ID"
    || -z "$owner_identity"
    || "${owner_scratch%/*}" != "$REPO_ROOT/artifacts/wave-b-mutations"
    || ! "$scratch_pid" =~ ^[1-9][0-9]*$
    || "$scratch_pid" != "$owner_pid" ]]; then
    echo "Refusing to reclaim Wave B mutation lock with mismatched owner identity." >&2
    return 1
  fi
  if [[ "$owner_pid" =~ ^[1-9][0-9]*$ ]] && kill -0 "$owner_pid" 2>/dev/null; then
    echo "Another Wave B mutation runner (PID $owner_pid) owns '$LOCK_DIR'." >&2
    return 1
  fi
  ...
  if [[ -n "$relative" ]]; then
    ...
    if [[ "$relative" == /* || "$relative" == *".."*
      ...
      echo "Refusing to reclaim malformed Wave B mutation recovery metadata." >&2
      return 1
    fi

    target="$REPO_ROOT/$relative"
    if [[ ! -f "$target" ]]; then
      echo "Refusing to restore a stale Wave B mutation whose target is missing." >&2
      return 1
    fi
    ...
    if [[ "$target_hash" == "$mutated_hash" ]]; then
      if [[ "$(shasum -a 256 "$backup" | awk '{print $1}')" != "$original_hash" ]]; then
        echo "Refusing to restore a stale Wave B mutation from a corrupt backup." >&2
        return 1
      fi
      local restore_path="$target.wave-b-stale-restore-$LOCK_TOKEN"
      cp -p "$backup" "$restore_path"
      mv -f "$restore_path" "$target"
      if [[ "$(shasum -a 256 "$target" | awk '{print $1}')" != "$original_hash" ]]; then
        echo "Stale Wave B mutation restoration did not reproduce the original bytes." >&2
        return 1
      fi
    ...
    if [[ "$staged" == "$target.wave-b-mutation-"* ]]; then
      rm -f "$staged"
    fi
  fi
  ...
  rm -rf "$stale"
  rm -rf "$owner_scratch"
  echo "Reclaimed stale Wave B mutation lock from PID ${owner_pid:-unknown}."
}
...
acquire_lock() {
  if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    ...
    mkdir "$LOCK_DIR" 2>/dev/null || {
      echo "Another Wave B mutation runner acquired '$LOCK_DIR'." >&2
      return 1
    }
  fi
...
for ((index = 0; index < COUNT; index++)); do
  ...
  CURRENT_STAGED="$CURRENT_FILE.wave-b-mutation-$LOCK_TOKEN"
